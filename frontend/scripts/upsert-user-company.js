...
async function main() {
  ...
  if (!target || !companyIdArg) {
    console.error(
      "Usage: node upsert-user-company.js <username|idClerk> <insuranceCompanyId>"
    );
    process.exit(1);
  }
  ...
  let user = await prisma.user.findFirst({
    where: { username: target, isDeleted: false },
  });
  if (!user) {
    user = await prisma.user.findFirst({
      where: { idClerk: target, isDeleted: false },
    });
  }

  if (!user) {
    console.log(
      `User not found for '${target}'. Creating a minimal user record linked to company ${insuranceCompanyId}.`
    );
  ...
  const updated = await prisma.user.update({
    where: { id: user.id },
    data: { insuranceCompanyId, lastModifiedBy: "script" },
  });
  console.log("Updated user:", {
    id: updated.id,
    username: updated.username,
    idClerk: updated.idClerk,
